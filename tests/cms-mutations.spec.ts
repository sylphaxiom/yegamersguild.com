...
test.describe('CMS mutations', () => {
  ...
  test('StringField saves and the save button reflects the completed round-trip', async ({ page }) => {
    ...
    try {
      // Edit and save — wait for the PUT response rather than sleeping
      await field.fill('CMS Test Value');
      const [response] = await Promise.all([
        page.waitForResponse(
          (r) => r.url().includes('/api/v1/content.php') && r.request().method() === 'PUT',
          { timeout: API_TIMEOUT },
        ),
        saveBtn.click(),
      ]);
      ...
      // After onSuccess → invalidateQueries → refetch, the field re-syncs with
      // the DB value and isDirty becomes false — Save button goes disabled again.
      // This proves the full round-trip (PUT → refetch → React update) completed.
      await expect(saveBtn).toBeDisabled({ timeout: API_TIMEOUT });
    } finally {
      // Always restore the original value, even if the assertion above failed
      await field.fill(original);
      await Promise.all([
        page.waitForResponse(
          (r) => r.url().includes('/api/v1/content.php') && r.request().method() === 'PUT',
