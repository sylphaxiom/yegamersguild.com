...
import type { Bullets } from "../About";
...
export default function BulletsField({ label, contentKey }: BulletsFieldProps) {
  ...
  const parsedValue = currentValue
    ? (JSON.parse(currentValue) as Bullets[])
    : [];
  const [dbValue, setDbValue] = React.useState<Bullets[]>(parsedValue);
  // Mutation
    ...
    >
      ...
      <Grid sx={{ alignSelf: "center" }}>
        <Button
          ...
          type="button"
          onClick={() =>
            setDbValue([...dbValue, { icon: "Sparkles", text: "" }])
          }
          disabled={isPending}
        >
          Add
